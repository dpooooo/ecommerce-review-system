...
export function buildReportSchema(options?: {
  ...
  userProfiles?: Array<Record<string, number | string>>;
  ...
  const reportTrafficSources = options?.trafficSources || trafficSources;
  const reportUserProfiles = options?.userProfiles || userProfiles;
  ...
  return {
    ...
    modules: [
      ...
      { key: "traffic_source", title: "流量来源分析", summary: "直播与搜索渠道 UV 价值更高，推荐流量承接效率偏低。", tables: [{ data: reportTrafficSources }] },
      { key: "user_profile", title: "用户经营分析", summary: "PLUS 与老客客单价更高，新客成交占比较高但复购仍需补强。", tables: [{ data: reportUserProfiles }] },
